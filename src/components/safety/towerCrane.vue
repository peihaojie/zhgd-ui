  ...
  <div id="towerCrane">
    <div class="content">
      ...
      <div class="content-box" v-for="(item,index) in towerCraneData" :key="index">
        <div class="top-box">
          <div class="status">
            <span :class="item.sb=='正常' || fake?'normal':'anomaly'">{{fake ? "正常" :item.sb}}</span>
          </div>
          <div class="employee">
            <span class="bolder">操作员</span>
            <span>{{item.name}}</span>
        ...
        <div class="bottom-box">
          ...
          <div class="day">
            ...
            <span
              class="bolder"
              :class="item.residue>=10 || fake ? 'normal' : item.residue >= 1 ? 'warning' : 'anomaly'"
            >{{fake ? 72 : item.residue}}天</span>
          </div>
        </div>
      </div>
...
export default {
  ...
  mounted() {
    let cid = sessionStorage.getItem("cid"); // 集团端id;
    if (cid == 1 || this.pid == 4) {
      this.fake = true;
    }
  },
