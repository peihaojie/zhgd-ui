        ...
        <div class="title">
          <div class="text">
            ...
            <router-link
              ...
              class="project-name"
              target="_blank"
              v-if="!$exe.installation"
            >{{indexData.projectName}}</router-link>
            <!-- 打包的时候打开注释 -->
            <router-link to="/spectaculars" class="project-name" v-else>{{indexData.projectName}}</router-link>
          </div>
          <!-- 完工=accomplish 正常=normal 停工=lockout -->
          <div
            class="state"
            :class="indexData.status==0?'normal':indexData.status==2?'accomplish':'lockout'"
          >{{indexData.status==0?'在建':indexData.status==2?'竣工':'停工'}}</div>
        </div>
        <div class="condition">
          <div class="worker">
            <div class="icon green-color"></div>
            <div class="text">
          ...
          .text {
            ...
            ul {
              ...
              li {
                ...
                &:nth-child(1) {
                  border-left: 0.02rem solid #0090ff;
                }
                &:nth-child(2) {
                  border-left: 0.02rem solid #04c366;
                }
                &:nth-child(3) {
                  border-left: 0.02rem solid #ffd14f;
                }
              }
            }
          }
...
export default {
  ...
  methods: {
    ...
    // 工资发放统计模块：ECharts图渲染
    grant() {
      let grant = this.$echarts.init(document.getElementById("grant"));
      grant.setOption({
        tooltip: {
          trigger: "axis",
          axisPointer: {
