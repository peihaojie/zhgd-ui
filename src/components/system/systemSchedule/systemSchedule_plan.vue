    ...
    <div class="content">
      ...
      <div class="main-list">
        <div class="main-list-item" v-for="(item,index) in planList" :key="index">
          ...
          <div class="main-list-button" :class="itemClick == index && item.nodeList &&item.nodeList.length > 0 ? 'block' : 'none'">
            <ul>
              <li class="first">
                ...
                <div class="operation">
                  <el-dropdown @command="nodeOperation">
                    ...
                    <el-dropdown-menu slot="dropdown">
                      ...
                      <el-dropdown-item :command="`delete:${item2.id}:${item2.nodeId}`">删除</el-dropdown-item>
                    </el-dropdown-menu>
                  </el-dropdown>
                </div>
              </li>
...
export default {
  ...
  methods: {
    ...
    // 删除进度计划
    remoProgressPlan(id) {
      if (!this.planList.filter((item) => item.id == id)[0].nodeList.length) {
        this.$axios
          .post(
    ...
    nodeOperation(command) {
      if (typeof command == "object") {
        ...
        if (command.split(":")[0] == "delete") {
          let id = command.split(":")[2]
          if (this.nodeList.filter(item => item.id == id)[0].end != null) {
            this.$message({
              type: 'warning',
              message: '该节点已完成，不可删除！'
            })
    ...
    search() {
      // console.log(this.dateValue);
      this.seachDateValue
        ? (this.startTime = this.seachDateValue[0])
        : (this.startTime = "");
      this.seachDateValue ? (this.endTime = this.seachDateValue[1]) : (this.endTime = "");
    ...
    importNodeList(planId) {
      ...
      let idList = list.filter(a=>tmpList.includes(a.id)).map(c=>c.id);
      ...
      this.addNodeList = list.filter(item => !idid.includes(item.id))
    }
  }
};
</script>