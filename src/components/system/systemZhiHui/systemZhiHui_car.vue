  ...
  <div id="systemZhiHui_car">
      ...
      <div id="down">
        <div id="left">
          <div class="btnList">
            ...
            <div class="btn" @click="selectTime = !selectTime">
          ...
          <div class="carInfo" v-show="!equipmentShow">
            ...
            <div class="carimg">
              <img v-if="newData" :src="newData.img" alt="" @click="imgShow(newData.img)" style="cursor: pointer">
              <div v-else></div>
            </div>
            <div class="infoBox">
              <ul>
                <li>
                  <span>驾驶员：</span>{{newData?newData.operatorin:''}}
                </li>
                <li>
                  <span>车牌号：</span>{{newData?newData.vehicleNo:''}}
                </li>
                <li>
                  <span>通道名称：</span>{{newData?newData.gateinname:''}}
                </li>
                <li>
                  <span>进出类型：</span>{{newData?(newData.inOut==1?'进': newData.inOut==2? '出' : ''):''}}
                </li>
                <li>
                  <span>车辆类型：</span>{{newData?(newData.cardType==0?'临时车':newData.cardType==1?'月租车':newData.cardType==2?'贵宾车':newData.cardType==3?'免费车':newData.cardType==4?'充值车':''):''}}
                </li>
                <li>
                  <span>抬杆方式：</span>
                  <span v-if="newData">{{newData.cardType==0?'手动抬杆':'自动抬杆'}}</span>
                </li>
                <li>
                  <span>进出时间：</span>{{newData?newData.liftTime:''}}
                </li>
              </ul>
            </div>
            <div class="status">
    ...
    <div class="dialog-box" v-show="selectTime">
        <div class="title">请选择时间
          <i class="el-icon-close" @click="selectTime=!selectTime"></i>
        </div>
        <div class="center">
          <span class="time">
            日期：
        ...
        <div class="footer">
          <div class="button" @click="down">确定</div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    // 查询车辆进出数据列表
    selectAll() {
      let {time, endTime} = {time: this.liftTime ? this.liftTime[0] : '', endTime: this.liftTime ? this.liftTime[1] : ''}
      this.$axios.post(`/api/parkings/selectAll?deptID=${this.deptID}&pageSize=${this.historyPageSize}&pageNum=${this.historyPageNum}&inOut=${this.inOut}&all=${this.all}&liftTime=${time}&endTime=${endTime}`).then(
        res => {
          // console.log(res.data)
    ...
    searchClick() {
      ...
      let {time, endTime} = {time: this.liftTime ? this.liftTime[0] : '', endTime: this.liftTime ? this.liftTime[1] : ''}
      this.$axios.post(`/api/parkings/selectAll?deptID=${this.deptID}&pageSize=${this.historyPageSize}&pageNum=${this.historyPageNum}&inOut=${this.inOut}&all=${this.all}&liftTime=${time}&endTime=${endTime}&sn=${this.sn}`).then(
        res => {
          // console.log(res.data)
          this.allTableData = res.data.data.rows
          this.historyPageTotal = res.data.data.total
    ...
    down() {
      if (this.times) {
        let {time, endTime} = {time: this.times[0], endTime: this.times[1]}
        location.href=`http://47.106.71.3:8080/api/parkings/export?deptID=${this.deptID}&liftTime=${time}&endTime=${endTime}`
      } else {
        this.$message({
          type: 'warning',
          message: '请选择下载时间段！'
        ...
        .searchBox {
          ...
          ul {
            ...
            li {
              ...
              /deep/.el-date-editor .el-range__icon, .el-date-editor .el-range-separator {
                line-height: .32rem;
              }
              /deep/.el-input__icon {
                line-height: .4rem;
