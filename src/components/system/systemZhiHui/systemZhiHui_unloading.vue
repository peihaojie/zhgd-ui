      ...
      <div class="main-box">
        ...
        <div class="real-time">
          ...
          <div class="bottom-box">
            ...
            <div class="top-box">
              <div style="background-color:#0090ff;" @click="getRealtimeHistory(2)">
              ...
              <div style="background-color:#3ada76;" @click="getRealtimeHistory(3)">
            ...
            <div class="bottom-box">
              <div style="background-color:#ffb079;" @click="getRealtimeHistory(1)">
              ...
              <div style="background-color:#0090ff;" @click="getRealtimeHistory(4)">
  ...
  methods: {
    ...
    getSbUnloaderRealtimeList(id) {
      ...
      this.$axios
        ...
        .then(res => {
          if (Reflect.ownKeys(res.data.data).length) {
    ...
    getRealtimeHistory(alarmType) {
      ...
      let {time, endTime} = {time: this.times ? this.times[0] : '', endTime: this.times ? this.times[1] : ''}
      this.$axios
        .post(`/api/unloaderPcApi/getRealtimeHistory?projectId=${this.projectId}&deviceId=${this.deviceId}&alarmType=${this.ListId}&time=${time}&endTime=${endTime}&pageNum=${this.pageNumAlarm}&pageSize=${this.pageSize}`)
        .then(res => {
    ...
    getSbUnloaderHistory() {
      let {time, endTime} = {time: this.times ? this.times[0] : '', endTime: this.times ? this.times[1] : ''}
      this.$axios
        .post(`/api/unloaderPcApi/getSbUnloaderHistory?projectId=${this.projectId}&deviceId=${this.deviceId}&pageNum=${this.pageNum}&pageSize=${this.pageSize}&time=${time}&endTime=${endTime}`)
        .then(res => {
    ...
    download() {
      if (this.times && this.times.length) {
        ...
        location.href=`http://47.106.71.3:8080/api/unloaderPcApi/exportUnloader?judge=${this.ListId}&deviceId=${this.deviceId}&time=${time}&endTime=${endTime}`
      } else {
        this.$message({
          type: 'warning',
          message: '请选择下载时间段！'
